...
3. ReadAnnotations : taxa and functional annoation of preprocessed reads 
            - if host_dbpath set in config then reads post host removal is run 
            - if host_dbpath is not set in config then reads post prinseq is run 
