...
rule kraken_phyla:
    ...
    output:
        temporary(os.path.join(RBADIR, "{sample}", "kraken", "{sample}.phyla_fraction.tsv"))
    shell:
        """
...
rule kraken_families:
    ...
    input:
        os.path.join(RBADIR, "{sample}", "kraken", "{sample}.report.tsv")
    output:
        temporary(os.path.join(RBADIR, "{sample}", "kraken", "{sample}.family_fraction.tsv"))
    shell:
        """
        echo "Family\t{wildcards.sample}" > {output};
        cat {input} | sed -e 's/Candidatus//' | \
...
rule kraken_genera:
    ...
    input:
        os.path.join(RBADIR, "{sample}", "kraken", "{sample}.report.tsv")
    output:
        temporary(os.path.join(RBADIR, "{sample}", "kraken", "{sample}.genera_fraction.tsv"))
    shell:
        """
        echo "Family\t{wildcards.sample}" > {output};
        cat {input} | sed -e 's/Candidatus//' | \
...
rule kraken_species:
    ...
    input:
        os.path.join(RBADIR, "{sample}", "kraken", "{sample}.report.tsv")
    output:
        temporary(os.path.join(RBADIR, "{sample}", "kraken", "{sample}.species_fraction.tsv"))
    shell:
        """
